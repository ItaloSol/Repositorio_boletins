...
<main class="container">
  ...
  <a href="/../boletins/2023/1_inserir_inserir.php" class="btn btn-warning rounded-pill px-3" type="button">Adicionar</a>
    <h6 class="border-bottom pb-2 mb-0">BOLETIM INTERNO</h6>
    <?php
$pasta = "X:/Programas/SISGRAFEx/WEB_2.0/boletins/2023";  // Substitua pelo caminho real da sua pasta

// Obtém a lista de arquivos na pasta
$arquivos = scandir($pasta);

// Filtra apenas os arquivos .pdf
$pdfs = array_filter($arquivos, function ($arquivo) use ($pasta) {
    return is_file($pasta . '/' . $arquivo) && pathinfo($arquivo, PATHINFO_EXTENSION) == 'pdf';
});

// Ordena os arquivos por nome (ordem natural)
natsort($pdfs);

// Exibe os arquivos
foreach ($pdfs as $pdf) {
    echo '<a href="visualizar_pdf.php?arquivo=' . urlencode($pdf) . '" target="_blank"><box-icon name="file-pdf" type="solid" ></box-icon>' . $pdf . '</a><br>';
}
